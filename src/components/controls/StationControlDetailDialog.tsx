...
import { StationControl, TarifItem, TarifBordItem } from '@/hooks/useSupabaseControls';
import { CitySelect } from './CitySelect';
import { GareSelector } from './GareSelector';
import { cn } from '@/lib/utils';

interface StationControlDetailDialogProps {
  control: StationControl;
  onUpdate?: (control: StationControl) => void;
...
export function StationControlDetailDialog({
  control,
  onUpdate,
  onDelete,
  trigger,
}: StationControlDetailDialogProps) {
  const [open, setOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [editedControl, setEditedControl] = useState<StationControl>(control);
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            <span>
              {isEditing ? 'Modifier le contrôle' : 'Détail du contrôle'} - {control.stationName}
            </span>
          </DialogTitle>
          <DialogDescription>
            {formatDate(control.date)} à {control.time} • Quai {control.platform}
            {control.origin && control.destination && ` • ${control.origin} → ${control.destination}`}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <GareSelector
                  id="edit-station"
                  value={editedControl.stationName}
                  onChange={(v) => setEditedControl({ ...editedControl, stationName: v })}
                />
              </div>
              <div className="space-y-2">
                <Label>Quai</Label>
                <Input
                  value={editedControl.platform}
                  onChange={(e) => setEditedControl({ ...editedControl, platform: e.target.value })}
                />
              </div>
              <div className="space-y-2">
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Cette action est irréversible. Le contrôle de la gare {control.stationName} sera définitivement supprimé.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
